...
export const MainDisplay:React.FC = () => {
    ...
    const setTubDataOnRedux = useCallback((tub:string) => {
        dispatcher(setTubData(tub))
    },[dispatcher])

    const setLetterDataOnRedux = useCallback((obj_list:Drink[]) => {
    ...
    const searchDataState = useSelector((state:RootState) => state.searchData.searchData)
    ...
    const handleTubChange = (event:SyntheticEvent, data:Object) => filterTabDataOnChange(data as TabData)
    ...
    const handleTubChangeFilter = (event:SyntheticEvent, data:Object) => filterTabDataOnChangeFilter(data as TabData)
    ...
    if(element === 1) {
        return (
            <section className={'---section-wrapper ---relative'}>
                <div className={width && width >= 1750 ? '---absolute position-search-bar' : '---absolute psb-mobile'}>
                    <SearchExampleStandard />
                </div>
                {
                    width && width >= 1750 ?
                        <>
                            <Tab panes={panes} className={'tab-layout'} onTabChange={handleTubChange}/>
                        </>
                        :
                        <>
                            <Tab menu={{ fluid: true, vertical: true, tabular: true }} panes={panes} className={'tab-layout tab-mobile'} onTabChange={handleTubChange}/>
                        </>
                }
            </section>
        )
        ...
        return(
            <section className={'---section-wrapper ---relative'}>
                ...
                </div>
                {
                    width && width >= 1750 ?
                        <>
                            <Tab panes={filter_panes} className={'tab-layout'} onTabChange={handleTubChangeFilter} />
                        </>
                        :
                        <>
                            <Tab menu={{ fluid: true, vertical: true, tabular: true }} panes={filter_panes} className={'tab-layout tab-mobile'} onTabChange={handleTubChangeFilter} />
                        </>
                }
        ...
        return (
            <div>
                Error... failed to define app state...
            </div>
        )
    }
